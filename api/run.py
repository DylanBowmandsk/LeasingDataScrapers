...
def createApp():
    ...
    def getPvMakes():
        ...
        cursor = db.cursor()
        makes = []
        for row in cursor.execute("select distinct manufacturer from [LeasingPrices] where not manufacturer = '' and type = 'Personal'"):
            if(len(row) > 0):
                for x in row:
                    makes.append(x)
    ...
    @app.route("/get/file")
    def getFile():
        return send_file("./list.csv", as_attachment=True)
    ...
    def getPvModels(make):
        ...
        cursor = db.cursor()
        models = []
        for row in cursor.execute("select distinct range from [LeasingPrices] where manufacturer ='"+make+"' and range not like '%TRANSIT%' and range not like '%DUCATO%' and type = 'Personal'"):
            if(len(row) > 0):
                for x in row:
                    models.append(x)
    ...
    def getPvVariants(make, model):
        ...
        cursor = db.cursor()
        variants = []
        for row in cursor.execute("SELECT Distinct model  FROM [dbo].[LeasingPrices] where range like '" + model + "%' and manufacturer = '"+make+"' and type = 'Personal'"):
           for x in row:
                if(len(row) > 0):
                    variants.append(x)
    ...
    def getPvDerivatives(model ,variant):
        ...
        cursor = db.cursor()
        derivatives = []
        for row in cursor.execute(f"SELECT Distinct derivative  FROM [dbo].[LeasingPrices] where model like '{variant}' and type = 'Personal'"):
            for x in row:
    ...
    @app.route("/scrape/all/<term>/<initial>/<mileage>")
    def scrapeEverything(term,initial,mileage):
        leaseLocoData = []
        selectData = []
        leasingData = []
        pvData = []
            ...
            for model in models:
                ...
                if variants != []:
                    ...
                    for variant in variants:
                        print(term,initial,mileage)
                        pvData = (json.loads(getAllPvPrice(model,variant,term,initial,mileage).data)) 
                        llt =CustomThread(target=scrapeAllLeaseLoco, args=[make,model,variant,str(term),str(initial),str(mileage)])
                        lt = CustomThread(target=scrapeAllLeasingcom, args=[make,model,variant,term,initial,mileage])
                        llt.start()
                        lt.start()
                        llt.join()
                        lt.join()
                        ...
                        for derivative in derivatives:
                            f = open('./listnew.csv', 'a', encoding='UTF8', newline='')
                            writer = csv.writer(f)
                            pvPrice = ""
                            locoPrice = ""
                            leasingPrice = ""
                            ...
                            writer.writerow([make, model, variant ,derivative, term, initial, mileage, "£"+pvPrice+"p/m", locoPrice+"p/m", leasingPrice])
                            f.close()
